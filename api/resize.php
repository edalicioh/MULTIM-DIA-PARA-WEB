...
$pastaDestino = __DIR__  .  "/../public/imgs/nova/";
...
if (is_dir($pastaOrigem)) {
    ...
    */
    ...
    for ($i = 0; $i < count($arquivos); $i++) {
        ...
        $nome = explode('/public/imgs/', $arquivos[$i])[1]; // Pega o nome original 

        $type = exif_imagetype($arquivos[$i]); // [] se você não tem exif, você pode usar getImageSize()

        // Salva o tamanho antigo da imagem
        list($largura_antiga, $altura_antiga) = getimagesize($arquivos[$i]);
